...
export default class OrderHandlerCache {
  ...
  static async getOrderFromMessage(msg_from: string): Promise<IOrder | null> {
    let order_json: string | null = null;
    try {
      order_json = await redisClient.get('order:' + msg_from);
    } catch (error) {
      console.error('Error while fetching order from cache: ', error);
      return null;
    }

    if (!order_json) {
      console.log('No order data found for user:', msg_from);
      return null; // No data found, return null
    }

    let order_obj: IOrder | null = null;

    try {
      // Try to parse the order_json safely
      order_obj = Convert.toIOrder(order_json);
    } catch (error) {
      console.error('Error while converting order to IOrder type:', error, order_json);
      return null; // Return null if there's an error
    }

    return order_obj; // Return parsed object if successful
  }
  ...
  static async setAtendimentoToFinish(nome_atendido: string): Promise<boolean> {
    const atendimento = await redisClient.get(`atendimento:554184510719@c.us:${nome_atendido}`);
    if (atendimento == null) {
      return false;
    }
