...
export const UpdateOrder = {
  async queryToUpdateOrder(order_id: number, status_to_update: string) {
    const result = await db('orders').where('id', order_id).update({ status: status_to_update });
  ...
  async updateOrderByStatus(order_id: number, status_to_update: string): Promise<string> {
    ...
    await OrderHandlerCache.setOder('order:' + chatId, JSON.stringify(obj_order));

    return chatId;
  },
  async updatePaymentByStatus(order_id: number, status_to_update: string): Promise<string> {
